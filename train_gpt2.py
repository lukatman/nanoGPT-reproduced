...
for i in range(50):
    ...
    # must set grads to 0 first, loss.backward() adds gradients to whatever they are
    optimizer.zero_grad() 
    logits, loss = model(x, y)
    loss.backward()
    optimizer.step()
    # loss - a tensor with a single element. .item() moves to cpu and converts to float
